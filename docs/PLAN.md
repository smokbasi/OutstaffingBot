...
**Обновление поста:** при `filled` / `cancelled` — edit message, добавить «❌ Закрыто». При наборе `workers_needed` принятых откликов — то же автозакрытие. При отмене отклика работником — снова открыть пост (убрать «Закрыто», вернуть кнопку), если заявка ещё `active` и набор не завершён.
